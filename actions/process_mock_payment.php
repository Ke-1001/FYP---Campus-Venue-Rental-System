...
{
    $stmt = $conn->prepare("
        SELECT bid, uid, status, payment_status, payment_due_at FROM booking
        WHERE bid = ? AND uid = ?
        FOR UPDATE
    ");
    ...
    if (empty($booking['payment_due_at']) || strtotime($booking['payment_due_at']) < time())
    {
        $cancel_stmt = $conn->prepare("
            UPDATE booking
            SET status = 'cancelled', cancelled_at = NOW(), cancel_reason = 'Payment deadline expired'
            WHERE bid = ? AND uid = ?
        ");
    ...
    if ($hasPaymentMethod && $hasPaidAt)
    {
        $stmt_update = $conn->prepare("
            UPDATE booking
            SET payment_status = 'paid', transaction_ref = ?, payment_method = ?, paid_at = NOW()
            WHERE bid = ? AND uid = ? AND status = 'pending' AND payment_status = 'unpaid'
        ");
    ...
    elseif ($hasPaymentMethod)
    {
        $stmt_update = $conn->prepare("
            UPDATE booking
            SET payment_status = 'paid', transaction_ref = ?, payment_method = ?
            WHERE bid = ? AND uid = ? AND status = 'pending' AND payment_status = 'unpaid'
        ");
        $stmt_update->bind_param("ssis", $transaction_id, $payment_method, $bid, $uid);
    }
    elseif ($hasPaidAt)
    {
        $stmt_update = $conn->prepare("
            UPDATE booking
            SET payment_status = 'paid', transaction_ref = ?, paid_at = NOW()
            WHERE bid = ? AND uid = ? AND status = 'pending' AND payment_status = 'unpaid'
        ");
    ...
    else
    {
        $stmt_update = $conn->prepare("
            UPDATE booking
            SET payment_status = 'paid', transaction_ref = ?
            WHERE bid = ? AND uid = ? AND status = 'pending' AND payment_status = 'unpaid'
        ");
        $stmt_update->bind_param("sis", $transaction_id, $bid, $uid);
    }
