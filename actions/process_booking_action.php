...
// This section checks booking decisions and updates the booking status.
session_start();
        ...
        foreach ($bids as $raw_bid)
        {
            ...
            if ($action === 'approve')
            {
                $sql_conflict = "
                    ...
                    WHERE vid = ? AND date_booked = ? AND time_start < ? AND time_end > ? AND status = 'approved'
                    LIMIT 1
                ";
                ...
                $sql_schedule_conflict = "
                    SELECT 1 FROM academic_schedule sch
                    JOIN semester_config sem ON sch.sem_id = sem.sem_id
                    WHERE sch.vid = ? AND sch.day_of_week = ? AND ? BETWEEN sem.start_date AND sem.end_date AND sem.is_active = 1 AND sch.start_time < ? AND sch.end_time > ?
                    LIMIT 1
                ";
