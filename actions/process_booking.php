...
try
{
    $user_stmt = $conn->prepare("
        SELECT uid, account_status FROM user
        WHERE uid = ?
        LIMIT 1
    ");
    ...
    $venue_stmt = $conn->prepare("
        SELECT vid, deposit, status FROM venue
        WHERE vid = ?
        LIMIT 1
    ");
    ...
    $sql = "
        INSERT INTO booking (uid, vid, date_booked, time_start, time_end, purpose, status, payment_status, payment_due_at)
