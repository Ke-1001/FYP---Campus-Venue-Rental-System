...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 // 1. Status and SLA validation (State Machine & SLA Validation)
        ...
        $critical_timestamp = $end_timestamp - 300; // T_end - 5min
 ...
 // 3. Run the minimum-load assignment rule (Greedy Minimum-Load Algorithm)
        if ($sid_raw === 'RA01') {
            $sql_ra01 = "
                ...
                FROM staff s
                LEFT JOIN (
                    SELECT i.sid, COUNT(*) as clash_count
                    FROM inspection i
                    JOIN booking b ON i.bid = b.bid
                    WHERE b.date_booked = ? AND b.time_end = ? AND i.ins_status = 'pending'
                    GROUP BY i.sid
                ) clash ON s.sid = clash.sid
