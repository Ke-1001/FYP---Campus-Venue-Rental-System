...
<?php endif; ?>
        ...
        <div class="w-full max-w-sm glass-panel rounded-2xl p-8 shadow-2xl">
            ...
            <form action="forgot_password_process.php" method="POST" class="space-y-4">
                <div>
                    ...
                    <input type="email" name="email" id="email" placeholder="student@student.mmu.edu.my" required class="w-full px-4 py-3 rounded-xl text-slate-800 font-semibold text-sm">
                </div>
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all">Send Reset Link</button>
            </form>
            <div class="mt-6 text-center">
                <a href="../User/user_login.php" class="text-slate-400 hover:text-white text-sm font-bold">Back to Login</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        emailInput.addEventListener('input', () => {
            ...
            // Logic: Disappear if empty, otherwise show only if invalid
