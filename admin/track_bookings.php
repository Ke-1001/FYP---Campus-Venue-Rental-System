...
|--------------------------------------------------------------------------
| When admin opens Track Bookings, expired unpaid bookings will be marked
...
if (!empty($filter_student)) {
    $sql .= " AND (
        u.username LIKE '%" . $conn->real_escape_string($filter_student) . "%' 
        OR u.uid LIKE '%" . $conn->real_escape_string($filter_student) . "%'
    )";
}

if (!empty($filter_venue)) {
    $sql .= " AND (
        v.vname LIKE '%" . $conn->real_escape_string($filter_venue) . "%' 
        OR vc.category LIKE '%" . $conn->real_escape_string($filter_venue) . "%'
    )";
}
...
function formatDateTimeAdmin($value) {
    ...
    return $timestamp ? date('Y/m/d H:i', $timestamp) : htmlspecialchars($value);
}

function statusBadgeClassAdmin($status) {
    return match ($status) {
        'pending' => 'bg-[#fffbeb] text-[#b45309] border-[#fde68a]',
        'approved' => 'bg-[#eff6ff] text-[#1d4ed8] border-[#bfdbfe]',
        'completed' => 'bg-[#ecfdf5] text-[#059669] border-[#a7f3d0]',
        'rejected' => 'bg-[#fef2f2] text-[#dc2626] border-[#fecaca]',
        'cancelled' => 'bg-[#f1f5f9] text-slate-600 border-[#cbd5e1]',
        default => 'bg-[#f1f5f9] text-slate-600 border-[#e2e8f0]'
    };
}

function paymentTextClassAdmin($paymentStatus) {
        ...
        <div class="flex-1 overflow-y-auto p-8 scroll-smooth">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm text-slate-600 mt-1">
                    View paid, unpaid, completed, rejected and cancelled venue booking records.
                </p>
            </div>
                ...
                <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
                    ...
                    <div>
                        ...
                        <input 
                            type="text" 
                            name="f_bid" 
                            value="<?php echo htmlspecialchars($filter_bid); ?>" 
                    ...
                    <div>
                        ...
                        <input 
                            type="text" 
                            name="f_venue" 
                            value="<?php echo htmlspecialchars($filter_venue); ?>" 
                    ...
                    <div class="flex space-x-2">
                        <button 
                            type="submit" 
                            class="w-full py-2 bg-[#004aad] text-white text-sm font-semibold rounded-md hover:bg-[#003882] transition shadow-sm border border-[#004aad]"
                        >
                            Apply
                        </button>

                        <a 
                            href="track_bookings.php" 
                            class="px-4 py-2 bg-transparent text-slate-600 text-sm font-semibold rounded-md hover:bg-slate-100 border border-transparent transition flex items-center justify-center" 
                            title="Reset Filters"
                        >
            ...
            <div class="custom-table-container">
                <div class="px-4 py-3 border-b border-slate-200 bg-[#f8fafc] flex justify-between items-center">
                    <h3 class="text-xs font-semibold text-slate-500 uppercase tracking-widest">
                        Records (<?php echo $result ? $result->num_rows : 0; ?>)
                    </h3>
                </div>
                        ...
                        <?php if ($result && $result->num_rows > 0): ?>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <?php
                                    ...
                                    $statusClass = statusBadgeClassAdmin($status);
                                    $paymentClass = paymentTextClassAdmin($paymentStatus);
                                ?>
                                ...
                                <tr ondblclick="window.location.href='process_flow.php?bid=<?php echo urlencode($row['bid']); ?>'">
                                    ...
                                    <td class="text-center">
                                        <div class="td-text-mono">
                                            ...
                                            <span class="block text-slate-500 font-bold">
                                                <?php echo $time_range; ?>
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-right">
                                        <div class="space-y-1.5 flex flex-col items-end">
                                            <span class="inline-block px-2 py-0.5 border <?php echo $statusClass; ?> rounded-sm text-[10px] font-bold uppercase tracking-widest text-center min-w-[85px]">
                                                <?php echo htmlspecialchars($status); ?>
                                            </span>
                                            
                                            <span class="inline-block text-[10px] font-bold uppercase tracking-widest <?php echo $paymentClass; ?>">
                                                Pay: <?php echo htmlspecialchars($paymentStatus); ?>
                                            </span>

                                            <?php if ($status === 'pending' && $paymentStatus === 'unpaid'): ?>
                                                <span class="block text-[10px] text-slate-500 font-semibold leading-tight max-w-[150px]">
                                                    Due:
                                                    <?php echo formatDateTimeAdmin($row['payment_due_at']); ?>
                                                </span>
                                            <?php endif; ?>

                                            <?php if ($status === 'cancelled'): ?>
                                                <span class="block text-[10px] text-slate-500 font-semibold leading-tight max-w-[170px]">
                                                    Cancelled:
                                                    <?php echo formatDateTimeAdmin($row['cancelled_at']); ?>
                                                </span>

                                                <span class="block text-[10px] text-slate-400 font-semibold leading-tight max-w-[170px]">
                                                    <?php echo htmlspecialchars($row['cancel_reason'] ?? 'Payment deadline expired'); ?>
                                                </span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-center align-middle">
                                        <a 
                                            href="process_flow.php?bid=<?php echo urlencode($row['bid']); ?>" 
                                            class="td-action-btn flex justify-center w-full"
                                        >
                                            &gt;
                                        </a>
                                    </td>
